<?php namespace Dotink\Flourish {
	...
	class Timestamp
	{
		...
		const callFormatCallback       = 'Dotink\Flourish\Timestamp::callFormatCallback';
		const callUnformatCallback     = 'Dotink\Flourish\Timestamp::callUnformatCallback';
		const combine                  = 'Dotink\Flourish\Timestamp::combine';
		const defineFormat             = 'Dotink\Flourish\Timestamp::defineFormat';
		const fixISOWeek               = 'Dotink\Flourish\Timestamp::fixISOWeek';
		const getDefaultTimezone       = 'Dotink\Flourish\Timestamp::getDefaultTimezone';
		const isValidTimezone          = 'Dotink\Flourish\Timestamp::isValidTimezone';
		const registerFormatCallback   = 'Dotink\Flourish\Timestamp::registerFormatCallback';
		const registerUnformatCallback = 'Dotink\Flourish\Timestamp::registerUnformatCallback';
		const reset                    = 'Dotink\Flourish\Timestamp::reset';
		const setDefaultTimezone       = 'Dotink\Flourish\Timestamp::setDefaultTimezone';
		const translateFormat          = 'Dotink\Flourish\Timestamp::translateFormat';

		//
		// Here's our regular constants
		//

		const ISO_WEEK_REGEX = '#^(.*)(\d{4})-W(5[0-3]|[1-4][0-9]|0?[1-9])-([1-7])(.*)$#D';
		 ...
		 * @internal
		 *
		 * @param string $formatted_string The formatted date/time/timestamp string to be modified
		 ...
		 * @internal
		 *
		 * @param string $date_time_string A raw date/time/timestamp string to be parsed/modified
		 ...
		 * @param string $formatting_string The format string compatible with PHP's date() function
		 * @return void
		 */
		...
		{
			if (preg_match(self::ISO_WEEK_REGEX, $date, $matches)) {
				$before = $matches[1];
				$year   = $matches[2];
				$week   = $matches[3];
				$day    = $matches[4];
				...
				$ymd = date(
					'Y-m-d',
					strtotime('+' . ($week-1) . ' weeks +' . ($day-1) . ' days', $iso_year_start)
				);
		 ...
		 *
		 * @param callback $callback  The callback to pass all formatted calls through
		 * @return void
		 */
		 ...
		 *
		 * @param  callback $callback  The callback to pass all date strings through
		 * @return void
		 */
		...
		static protected function compose($message)
		{
			...
			if (class_exists(__NAMESPACE__ . '\Text', FALSE)) {
				return Text::create($message)->compose(NULL, $args);
			} else {
		...
		{
			if (!Core::checkVersion('5.1')) {
				throw new EnvironmentException(
					'The %s class takes advantage of the timezone features in PHP 5.1.0 and'
					. ' newer. Unfortunately it appears you are running an older version of'
					. ' PHP.',
					__CLASS__
				);
			}
		 ...
		 *
		 * @throws ValidationException  When `$datetime` is not a valid date/time, date or time value
		 *
		 * @param  Timestamp|object|string|integer $datetime  The date/time to represent, `NULL` is interpreted as now
		 ...
		 *
		 * @param  Timestamp|string|integer $other_timestamp  The timestamp to create the difference with, `NULL` is interpreted as now
